...
class SkipDbMiddleware(BaseHTTPMiddleware):
    """Disable non-essential DB-reliant endpoints when SKIP_DB is True."""
    ...
    async def dispatch(
        ...
        if settings.SKIP_DB:
            ...
            # Paths starting with these prefixes are allowed
            allowed_prefixes = (
                f"{root_prefix}/v1/voice",
                f"{root_prefix}/v1/auth/otp/request",
                f"{root_prefix}/v1/auth/otp/verify",
                f"{root_prefix}/v1/auth/google/verify",
                f"{root_prefix}/v1/health",
                f"{root_prefix}/docs",
                f"{root_prefix}/redoc",
                f"{root_prefix}/openapi.json",
            )

            # Allow exactly the root paths
            exact_allowed = {root_prefix, "/", f"{root_prefix}/"}

            if path not in exact_allowed and not path.startswith(allowed_prefixes):
                # Mock profile-photo/signed-url to prevent frontend errors
                if path.startswith(f"{root_prefix}/v1/doctors/") and path.endswith("/profile-photo/signed-url"):
                    return JSONResponse(
                        status_code=200,
                        content={
                            "message": "Mock photo URL",
                            "data": {
                                "signed_url": "",
                                "expires_in": 0
                            }
                ...
                return JSONResponse(
                    ...
                    content={
                        "error": {
                            ...
                            "message": "SKIP_DB is enabled. This endpoint relies on the database and is disabled in mock mode."
                        }
                    }
                )
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    if settings.SKIP_DB:
        logger.warning("SKIP_DB is enabled — skipping database health check at startup")
    else:
...
All endpoints are versioned under `/api/v1/`.
    ...
    # 2b. Skip DB Middleware
    app.add_middleware(SkipDbMiddleware)
