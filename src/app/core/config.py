...
class Settings(BaseSettings):
    ...
    LOG_LEVEL: Literal["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"] = Field(  # type: ignore
        default="INFO",
    ...
    SKIP_DB: bool = Field(
        default=False,
        description=(
            "When True, bypasses database requirements, allowing the app to run "
            "for testing the voice conversational bot. Disables all other DB-reliant endpoints. "
            "MUST be False (or absent) in production."
        ),
    )
    ...
    def validate_production_settings(self) -> "Settings":
        ...
        if self.is_production:
            ...
            if self.SKIP_DB:
                raise ValueError(
                    "SKIP_DB must be False in production. "
                    "This flag is only for development/testing."
                )
        return self
