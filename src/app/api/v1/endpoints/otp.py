...
async def verify_otp(
    ...
    if settings.SKIP_DB:
        doctor_id = 999999
        doctor_email = "mock@example.com"
        user_role = "user"
        is_new_user = False
        doctor_phone = request.mobile_number
        logger.info("SKIP_DB enabled — returning mock database user", doctor_id=doctor_id)
    else:
...
async def google_verify(
    ...
    if settings.SKIP_DB:
        ...
        doctor_id = doctor.id
        doctor_phone = doctor.phone or None
