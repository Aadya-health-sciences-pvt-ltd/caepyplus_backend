...
async def get_db() -> AsyncGenerator[AsyncSession | None, None]:
    ...
    When SKIP_DB is enabled (dev/test mode), yields ``None`` so that endpoint
    handlers can check ``settings.SKIP_DB`` and skip all DB operations without
    SQLAlchemy ever attempting to connect to (or even parse) the database URL.
    """
